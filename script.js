...
// Simple clothing animation
document.addEventListener('DOMContentLoaded', function () {
    const canvas = document.getElementById('clothing-animation');
    if (!canvas) return;
    ...
    function drawFigure() {
        ...
        // Head
        ctx.fillStyle = '#f4c2a3';
        ctx.beginPath();
        ctx.arc(100, 30, 15, 0, Math.PI * 2);
        ctx.fill();

        // Shirt
        const shirtColor = `hsl(${Math.random() * 360}, 60%, 55%)`;
        ctx.fillStyle = shirtColor;
        ctx.fillRect(80, 45, 40, 40);

        // Bottom: pants or shorts
        ctx.fillStyle = '#444';
        if (Math.random() < 0.5) {
            // Pants
            ctx.fillRect(80, 85, 40, 40);
        } else {
            // Shorts with legs
            ctx.fillRect(80, 85, 40, 20);
            ctx.fillRect(80, 105, 15, 30);
            ctx.fillRect(105, 105, 15, 30);
        }

        // Optional book bag
        if (Math.random() < 0.3) {
            ctx.fillStyle = '#8b4513';
            ctx.fillRect(60, 45, 20, 30);
        }
    }

    drawFigure();
    setInterval(drawFigure, 2000);
});
